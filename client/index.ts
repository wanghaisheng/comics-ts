...
window.addEventListener('load', async function () {
  try {
    ...
    comics.forEach((comic) => {
      ...
      var comicElement = document.createElement('article')
      var html = `<h2><a href="${comic.linkUrl}">${comic.name}</a></h2>`
      if (comic.data.img) html += `<p><img src="${comic.data.img}"></p>`
      if (comic.data.img2) html += `<p><img src="${comic.data.img2}"></p>`
      if (comic.data.title) html += `<p>${comic.data.title}</p>`
      comicElement.innerHTML = html
